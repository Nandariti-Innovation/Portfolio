...
export const Header = () => {
  ...
  const { darkTheme, setDarkTheme, mobileOpen, setMobileOpen } = useDashboardUi();
  ...
  return <header className="relative z-40 flex h-navbar shrink-0 items-center justify-between border-b border-gray-200 bg-white px-3 text-gray-900 shadow-sm dark:border-gray-700 dark:bg-gray-900 dark:text-white sm:px-5">
    <div className="flex min-w-0 items-center gap-3">
      <button type="button" onClick={() => setMobileOpen(!mobileOpen)} aria-label="Open dashboard navigation" aria-expanded={mobileOpen} className="grid size-10 shrink-0 cursor-pointer place-items-center rounded-xl border border-gray-200 text-gray-600 hover:bg-gray-50 lg:hidden dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-800"><Menu size={20} /></button>
    ...
    <div className="flex items-center gap-2">
      ...
      <button type="button" onClick={() => setDarkTheme(!darkTheme)} aria-label={darkTheme ? "Use light theme" : "Use dark theme"} title={darkTheme ? "Use light theme" : "Use dark theme"} className="grid size-9 cursor-pointer place-items-center rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-800">{darkTheme ? <Sun size={17} /> : <Moon size={17} />}</button>
