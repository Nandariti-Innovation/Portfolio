  ...
  return <div className="space-y-1.5">
    ...
    <div className="space-y-2">
      {items.map((item, index) => <div className="relative" key={`${heading}-${index}`}>
        <input type="text" value={item} onChange={(e) => updateFeatures(items.map((value, i) => i === index ? e.target.value : value))} placeholder="Add a concise item" required
        ...
        <button type="button" aria-label={index === items.length - 1 ? "Add item" : "Remove item"}
          onClick={() => updateFeatures(index === items.length - 1 ? [...items, ""] : items.filter((_, i) => i !== index))}
