...
export const HeadingsEditor = ({ onUnsavedChange }: { onUnsavedChange: (unsaved: boolean) => void }) => {
  ...
  return <section aria-labelledby="headings-title" className="space-y-5">
    <div className="flex flex-wrap items-start justify-between gap-4">
      ...
      <div className="text-right"><button type="button" disabled={loading || !saved || busy || changed || editing} onClick={() => setCreating(true)}
      ...
      : <>
        <div className="grid gap-5 xl:grid-cols-[215px_minmax(0,1fr)]">
          <div className="flex gap-2 overflow-x-auto xl:flex-col" aria-label="Homepage sections">
            {ordered.map((item) => <button type="button" key={item.section_key} onClick={() => {
              if (item.section_key === selected) return;
              if (changed && !window.confirm("Discard unsaved heading changes?")) return;
              if (changed && saved) setDraft(structuredClone(saved.setting_object));
              setSelected(item.section_key); setEditing(false); setError("");
            }}
          ...
          {section && <div className="space-y-5">
            <article className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-800 sm:p-6">
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-[100px_minmax(0,1fr)]">
                ...
                <label className="text-sm font-medium">Eyebrow<input disabled={!editing} className={inputClass} maxLength={40} value={section.heading.eyebrow} onChange={(event) => editHeading("eyebrow", event.target.value)}/><span className="mt-1 block text-right text-xs text-gray-400">{section.heading.eyebrow.length}/40</span></label>
              </div>
              <label className="mt-3 block text-sm font-medium">Title<input disabled={!editing} className={inputClass} maxLength={160} value={section.heading.title} onChange={(event) => editHeading("title", event.target.value)}/><span className="mt-1 block text-right text-xs text-gray-400">{section.heading.title.length}/160</span></label>
              <div className="mt-5 grid gap-4 border-t border-gray-100 pt-5 dark:border-gray-700 sm:grid-cols-2 sm:items-end">
                <label className="text-sm font-medium">Display order<input disabled={!editing} type="number" min={1} step={1} className={inputClass} value={section.order} onChange={(event) => patch({ order: Number(event.target.value) })}/></label>
                <label className="flex items-center gap-3 rounded-xl border border-gray-200 p-3 text-sm dark:border-gray-700">
                  <input type="checkbox" checked={section.enabled} disabled={!editing || !section.template_key || !templates.some(t => t.template_key === section.template_key && t.is_published)} onChange={(event) => patch({ enabled: event.target.checked })}/>
                ...
                {isV2Template(templates.find(t => t.template_key === section.template_key)) ?
                  ...
                  {templates.find(t => t.template_key === section.template_key)!.slots.map(slot => {
                    ...
                    return <label key={slot.key} className="text-sm capitalize">{slot.key.replaceAll("_", " ")}{slot.required ? " *" : ""}
                      <select disabled={!editing} className={inputClass} value={field} onChange={e => {
        ...
        {editing && <div className="sticky bottom-0 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-gray-200 bg-white/95 p-3 shadow-lg backdrop-blur dark:border-gray-700 dark:bg-gray-800/95">
