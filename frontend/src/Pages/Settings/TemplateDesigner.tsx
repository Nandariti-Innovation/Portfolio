...
export function TemplateDesigner({ original, onClose, onSaved }: { original: SectionTemplate | null; onClose: () => void; onSaved: () => void }) {
  ...
  const save = async (data: PuckTemplateData) => {
    ...
    try { slots = analyzeTemplate(data); } catch (issue) { setError(issue instanceof Error ? issue.message : "Invalid layout."); return; }
    setBusy(true); setError("");
    const definition = { variant: "puck", schema_version: 1, show_heading: data.content.some(node => node.type === "SectionHeading"),
      fields: slots.map(slot => slot.key), puck_data: data };
    const patch = { display_name: name.trim(), description: description.trim(), layout_key: "puck",
      display_fields: slots.map(slot => slot.key), layout_definition: definition, slots, is_published: published, updated_at: new Date().toISOString() };
  ...
  return <div className="p-4 dark:text-white">
    ...
    <div className="mt-4 grid gap-3 sm:grid-cols-[1fr_1fr_2fr]">
      ...
      <label className="text-sm">Description<input className={input} value={description} maxLength={500} onChange={e => setDescription(e.target.value)}/></label>
    </div>
    <div className="my-3 flex items-center justify-between gap-3"><p className="text-xs text-gray-500">Data slots are mapped to table columns in Settings → Headings after publishing.</p>
